...
class Cortex {
            ...
            socket.on('message', (data)=>{
                try {
                    if(JSON.parse(data)['id']==CONTROL_DEVICE_ID){
                        ...
                        // resolve(data) contains the 'eq' and 'pow'
                    }
                } catch (error) {}
            })
        }) 
    ...
    addMarker(){
        this.socket.on('open',async ()=>{
            ...
            //numTran temporary, delete in error
            let numTrain;
    ...
    sub(streams){
        this.socket.on('open',async ()=>{
            ...
            this.socket.on('message', (data1)=>{
                ...
                //datapow = JSON.parse(streams);
                //data = data.trim();
               let data = JSON.parse(data1);
              // {"pow":[114.812,44.392,53.324,62.159,139.106,2.216,2.652,6.987,14.715,65.967,239.706,149.523,202.885,294.949,1360.159,59.194,15.695,11.399,1.991,11.137,80.421,11.824,4.69,2.368,7.137],"sid":"ce978c04-bc53-4461-b885-2ac11aa32ec3","time":1657822914.9726}
                // log stream data to file or console here
               
                //console.log(data1)

                //console.log('EEG Data', EEGData);
                /*for (let i = 0; i < overallEEGQuality.length; i++){
                    console.log('EQ Data', overallEEGQuality[i].y)
                }*/
                console.log('EQ Data', overallEEGQuality.y)
                
                //console.log('Insight overall EEG quality', overallEEGQuality.y.output[''])
                //Look at w3schools json
                ...
                // Look up time variable
                //time += 0.1;
                time += 1;

               /* if (overallEEGQuality.y >= 70){
                    tokenCount += 1;
                }
...
// ---------------------------------------------------------
//ws for local projects and wss for business applications for security
// ---------- sub data stream
// have six kind of stream data ['fac', 'pow', 'eeg', 'mot', 'met', 'com']
// 'dev' Contains data such as battery level, the wireless signal strength, and the contact quality of each EEG sensor.
// user could sub one or many stream at once
// dev: ['AF3', 'T7', 'Pz', 'T8', 'AF4', 'OVERALL']

let CortexSocketUrl = 'wss://localhost:6868';
let user = {
    "license":"",
    "clientId":"MwiUyq5sCJH00XwDvJ2CjMJ4I2kH9IT5l9VPe0Le",
    "clientSecret":"6FMgDmJlJ9YdMUv5knPC2Rs31cNgO71wSlkhefLuuZidsGXfbYNGHP8U9tvq8hIMrJ4nh7sotXNXjALbOEP1NTe28lfJ5Mpnyg01rJTiu4bG6jqobKs4kzShP3UNVjUT",
    "debit":100
}

let c = new Cortex(user, CortexSocketUrl)
